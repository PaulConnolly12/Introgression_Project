...
# DATA PREPARATION
data_prep <- function(prefix,chr,window_type,window_size){
...
# A function to return the source directory of the script when run
thisFile <- function() {
  cmdArgs <- commandArgs(trailingOnly = FALSE)
  needle <- "--file="
  match <- grep(needle, cmdArgs)
  if (length(match) > 0) {
    # When run via Rscript from the command line
